...
/**
 * @returns The scene nodes in the current selection, sorted front to back (top to bottom in the scene tree UI).
 */
function getSortedSelection(): SceneNode[] {
  ...
  function walk(node: BaseNode) {
    ...
    if ("children" in node) {
      ...
      // BECAUSE the `children` property lists the layers sorted back to front (bottom to top in scene tree ui)
